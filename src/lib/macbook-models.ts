...
/**
 * Сколько моделей в линейке по `public/macbook/{lineName}/` (только FS, без БД).
 * Совпадает с тем, что отдаёт разводка на странице линейки.
 */
export function countMacModelsInLine(lineName: string): number {
  const linePath = path.join(BASE, lineName);
  try {
...
export function getMacbookLines(): MacbookLine[] {
  ...
  try {
    ...
    for (const e of entries) {
      ...
      const count = countMacModelsInLine(e.name);
      lines.push({
        lineName: e.name,
  ...
  return lines.sort((a, b) => {
    const ra = ORDER[a.lineName] ?? 999;
    const rb = ORDER[b.lineName] ?? 999;
    if (ra !== rb) return ra - rb;
    return a.lineName.localeCompare(b.lineName, "ru");
  });
}
