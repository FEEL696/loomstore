...
/** Одна строка на пару (цвет, память); при дублях в БД (например, разный sim_type) — строка с минимальным id. */
function dedupeIphoneRowsByColorMemory(rows: TableRow[]): TableRow[] {
  const map = new Map<string, TableRow>();
  for (const r of rows) {
    if (!("memory_gb" in r) || !("color" in r)) continue;
    const key = `${r.color}\0${r.memory_gb}`;
    const prev = map.get(key);
    const id = r.id;
    if (!prev || id < prev.id) map.set(key, r);
  }
...
async function buildIndex(): Promise<Index> {
  ...
  for (const category of categories) {
    ...
    for (const [baseTitle, group] of byBaseTitle) {
      ...
      const variantRows =
        category === "iphone" ? dedupeIphoneRowsByColorMemory(group) : group;
...
> {
  ...
  const out = {} as Record<ProductCategory, number>;
  for (const c of categories) {
    out[c] = idx.productsByCategory.get(c)?.length ?? 0;
  }
  return out;
}
...
/** Поиск по названию и slug; iPhone/iPad/MacBook — страница модели в каталоге, остальное — карточка товара. */
export async function searchProducts(
  query: string,
  limit = 12
): Promise<SearchProductHit[]> {
  const needleRaw = normalizeSearchQuery(query);
  if (needleRaw.length < 2) return [];

  const idx = await getIndex();
  const hits: SearchProductHit[] = [];

  for (const product of idx.productsBySlug.values()) {
    const titleNorm = normalizeSearchQuery(product.title);
    const slugNorm = normalizeSearchQuery(product.slug.replace(/-/g, " "));
    if (!titleNorm.includes(needleRaw) && !slugNorm.includes(needleRaw)) continue;

    let href: string;
    if (product.category === "iphone") {
      href = `/category/iphone/${product.slug}`;
    } else if (product.category === "ipad") {
      href = `/category/ipad/${product.slug}`;
    } else if (product.category === "macbook") {
      href = `/category/macbook/${product.slug}`;
    } else {
  ...
  hits.sort((a, b) => a.title.localeCompare(b.title, "ru"));
  return hits.slice(0, limit);
}

export async function getAllProductSlugs(): Promise<string[]> {
  const idx = await getIndex();
  return Array.from(idx.productsBySlug.keys()).sort((a, b) =>
