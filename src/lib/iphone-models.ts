...
export function getColorsForModel(modelSlug: string): string[] {
  ...
  try {
    ...
    /** Одна запись на каноническое имя (NFC), чтобы не дублировать цвет при разной нормализации Unicode. */
    const byNfc = new Map<string, string>();
    for (const e of dirs) {
      const nfc = e.name.normalize("NFC");
      if (!byNfc.has(nfc)) byNfc.set(nfc, e.name);
    }
    const colors = [...byNfc.values()].sort((a, b) =>
